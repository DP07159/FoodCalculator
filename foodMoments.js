...
async function load(){try{const r=await AuthShell.request('/food-moments');const text=await r.text();let items=null;try{items=text?JSON.parse(text):null;}catch{throw new Error(`Food Moments konnten nicht geladen werden (ungültige Serverantwort, HTTP ${r.status}).`);}if(!r.ok)throw new Error(items?.error||`Food Moments konnten nicht geladen werden (HTTP ${r.status}).`);allMoments=Array.isArray(items)?items:[];render();}catch(e){list.innerHTML=`<p>${esc(e.message)}</p>`;}}
